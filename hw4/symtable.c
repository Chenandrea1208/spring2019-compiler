...
int insertTableNode(struct SymTable *table,struct SymTableNode* newNode)
{
	...
	{
		if(searchTable_wheninsert(table,newNode->name)==1) {
			//printf("correct\n");
...
int searchTable_wheninsert(struct SymTable* table,char* name)
{
	struct SymTableNode* first = table->head;
	while(first != NULL) {
...
/*new*/
struct SymTableNode* searchTable_Whenrefer(struct SymTable* table,char* name)
{
	struct SymTableNode* first = table->head;
	while(table != NULL) {
		if(table->tail != NULL){
	...
	ERROR_t=1;fprintf(stderr, "##########Error at Line %d: %s undeclared.##########\n", linenum,name);
	return NULL;
}

void insert_expresslist(struct Express* express,struct ExpressList* list)
{
	...
	if(list->len==0) {
		...
		struct Express* cur_exp = list->head;
		for(int i=1; i < list->len; i++) {
			cur_exp = cur_exp->next;
		}
		cur_exp->next = express;
		list->len++;
	}
}
void parameter_check(struct ExpressList* list,struct SymTableNode*node) 
{
	if(list!=NULL) {
		int num;
		if(node->attr != NULL && node->kind == FUNCTION_t) num = node->attr->funcParam->paramNum;
		else num = 0;

		if(node->kind!=FUNCTION_t) {
			ERROR_t=1;fprintf(stderr, "##########Error at Line %d: it is not function.##########\n", linenum);
		}
		else if(list->len==num) {
			struct Express* express = list->head;
			struct FuncAttrNode* fattr = node->attr->funcParam->head;
			for(int i = 0;i<list->len;i++) {
				fattr->value;
				if(fattr->value->isArray != express->isArray) {
					ERROR_t=1;fprintf(stderr, "##########Error at Line %d: parameter type error.##########\n", linenum);
					break;
				}
				else if(fattr->value->dim != express->dim) {
					ERROR_t=1;fprintf(stderr, "##########Error at Line %d: parameter type error.##########\n", linenum);
					break;
				}
				else {
					int error = 0;
					BTYPE type1 = fattr->value->baseType, type2 = express->baseType;
					switch(type1)
					{
						case INT_t:
							if(type2!=INT_t){
								error = 1;
								ERROR_t=1;fprintf(stderr, "##########Error at Line %d: type error.##########\n", linenum);
							}
							break;
						case FLOAT_t:
			...
			ERROR_t=1;fprintf(stderr, "##########Error at Line %d: parameter number unmatch.##########\n", linenum);
		}
	}
	else {
		if (node->attr != NULL){
			ERROR_t=1;fprintf(stderr, "##########Error at Line %d: parameter unmatch.##########\n", linenum);
		}
	}
}
